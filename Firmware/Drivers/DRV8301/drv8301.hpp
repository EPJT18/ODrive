...
#include <MotorControl/component.hpp>


class Drv8301 : public GateDriverBase, public OpAmpBase, public ComponentBase {
public:
    typedef enum {
     ...
     */
    bool is_ready(uint32_t timestamp) final;
    ...
    /**
     * @brief Starts a liveness check. The liveness check runs asynchronously
     * using the SPI arbitrator.
     * If the application fails to call this function periodically, is_ready()
...
private:
    ...
    void on_spi_complete(bool success);
    ...
    uint16_t tx_buf_sync_, rx_buf_sync_, tx_buf_async_, rx_buf_async_;

    bool is_ready_ = false;

    // Liveness checks
    Stm32SpiArbiter::SpiTask spi_task_; //!< SPI task for polling the DRV8301 liveness
    bool spi_task_active_ = false; //!< used to prevent concurrent double use of `spi_task_`
    uint32_t next_liveness_timestamp_ = 0;
    uint32_t last_liveness_timestamp_ = 0;
};
