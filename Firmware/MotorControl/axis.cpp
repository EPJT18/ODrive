...
bool Axis::start_closed_loop_control() {
    ...
    CRITICAL_SECTION() {
        if (sensorless_mode) {
            ...
            if (controller_.config_.setpoints_in_cpr) {
                controller_.pos_estimate_src_ = &ax->encoder_.pos_cpr_;
                controller_.pos_wrap_src_ = &ax->encoder_.config_.cpr;
            } else {
                controller_.pos_estimate_src_ = &ax->encoder_.pos_estimate_;
                controller_.pos_wrap_src_ = nullptr;
            }
        ...
        controller_.pos_setpoint_ = controller_.pos_estimate_src_ ? *controller_.pos_estimate_src_ : 0.0f;
        controller_.input_pos_ = controller_.pos_estimate_src_ ? *controller_.pos_estimate_src_ : 0.0f;
