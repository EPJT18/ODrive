...
public:
    std::variant<std::array<float, 3>, ODriveIntf::MotorIntf::Error> calculate(
            float vbus_voltage, std::array<float, 3> currents,
            uint32_t input_timestamp, uint32_t output_timestamp) final;

    virtual std::variant<std::tuple<float, float>, ODriveIntf::MotorIntf::Error> calculate(
...
public:
    ...
    // These values (or some of them) are updated inside calculate()
