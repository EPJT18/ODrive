...
std::variant<std::array<float, 3>, Motor::Error> AlphaBetaFrameController::calculate(
            float vbus_voltage, std::array<float, 3> currents,
            uint32_t input_timestamp, uint32_t output_timestamp) {
    // Clarke transform
    float Ialpha = currents[0];
    float Ibeta = one_by_sqrt3 * (currents[1] - currents[2]);

    auto result = calculate(vbus_voltage, Ialpha, Ibeta, input_timestamp, output_timestamp);
...
std::variant<std::tuple<float, float>, Motor::Error> FieldOrientedController::calculate(
    ...
    if ((int32_t)(output_timestamp - timestamp_) < 0) {
        return Motor::ERROR_BAD_TIMING;
    } else if (output_timestamp - timestamp_ > (uint32_t)((uint64_t)max_delay_us * (uint64_t)TIM_1_8_CLOCK_HZ / 1000000ULL)) {
        return Motor::ERROR_BAD_TIMING;
    }
