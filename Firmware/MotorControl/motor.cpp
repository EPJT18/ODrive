...
struct ResistanceMeasurementCotnrolLaw : AlphaBetaFrameController {
    std::variant<std::tuple<float, float>, ODriveIntf::MotorIntf::Error> calculate(
        float vbus_voltage, float Ialpha, float Ibeta,
        uint32_t input_timestamp, uint32_t output_timestamp) final
    {
...
struct InductanceMeasurementCotnrolLaw : AlphaBetaFrameController {
    std::variant<std::tuple<float, float>, ODriveIntf::MotorIntf::Error> calculate(float vbus_voltage,
        float Ialpha, float Ibeta,
        uint32_t input_timestamp, uint32_t output_timestamp) final
    {
        if (attached_) {
            deltaI_ += -sign_ * (Ialpha - last_Ialpha_);
            sign_ = -1.0f * sign_;
        } else {
        ...
        float vfactor = 1.0f / ((2.0f / 3.0f) * vbus_voltage);
        return std::make_tuple(sign_ * test_voltage_ * vfactor, 0.0f);
    }
    ...
    float get_inductance() {
        ...
        return test_voltage_ / (deltaI_ / dt);
    }
...
bool Motor::setup() {
    ...
    // Set trip level
    overcurrent_trip_level_ = (kTripMargin / kMargin) * max_allowed_current_;
...
bool Motor::do_checks(uint32_t timestamp) {
    if (!gate_driver_.is_ready(timestamp)) {
...
void Motor::tim_update_cb(uint32_t adc_a, uint32_t adc_b, uint32_t adc_c) {
    ...
    // Check if all expected ADC readings are valid
    bool current_sensor_error_ = ((current_sensor_mask_ & 1) && (adc_a == 0xffffffff))
                              || ((current_sensor_mask_ & 2) && (adc_b == 0xffffffff))
                              || ((current_sensor_mask_ & 4) && (adc_c == 0xffffffff));
    if (current_sensor_error_) {
        set_error(ERROR_CURRENT_SENSOR_ERROR);
        return;
    }

    // Convert ADC readings to current values
    Iph_ABC_t current = {
        (current_sensor_mask_ & 1) ? phase_current_from_adcval(adc_a) : NAN,
        (current_sensor_mask_ & 2) ? phase_current_from_adcval(adc_b) : NAN,
        (current_sensor_mask_ & 4) ? phase_current_from_adcval(adc_c) : NAN
    };
    ...
    switch (current_sensor_mask_) {
        ...
        case 0b111: break;
        default: set_error(ERROR_CURRENT_SENSOR_ERROR); return;
    }
    ...
    if (was_current_sense) {
        ...
        last_current_meas_timestamp_ = last_update_timestamp_;

        if (!(abs(I_leak_) < config_.I_leak_max)) {
            set_error(ERROR_I_LEAK_OUT_OF_RANGE);
            return;
        }
        ...
        current_meas_valid_ = 
            (!(current_sensor_mask_ & 1) || (abs(current_meas_.phA) < overcurrent_trip_level_)) &&
            (!(current_sensor_mask_ & 2) || (abs(current_meas_.phB) < overcurrent_trip_level_)) &&
            (!(current_sensor_mask_ & 4) || (abs(current_meas_.phC) < overcurrent_trip_level_)) &&
    ...
    if (should_update_pwm) {
        ...
        if (is_armed_) {
            std::variant<std::array<float, 3>, Error> control_law_result = ERROR_CONTROLLER_FAILED;
            if (control_law_) {
                control_law_result = control_law_->calculate(vbus_voltage,
                        {current_meas_.phA, current_meas_.phB, current_meas_.phC},
                        last_current_meas_timestamp_, last_update_timestamp_ + 2 * TIM_1_8_PERIOD_CLOCKS * (TIM_1_8_RCR + 1));
            }

            if (control_law_result.index() == 0) {
                ...
                i_bus = (0.5f - pwm_timings[0]) * current.phA + (0.5f - pwm_timings[1]) * current.phB + (0.5f - pwm_timings[2]) * current.phC;
                ...
                set_error(std::get<1>(control_law_result));
                return;
            }
            ...
            i_bus = (pwm_timings[0] - 0.5f) * current.phA + (pwm_timings[1] - 0.5f) * current.phB + (pwm_timings[2] - 0.5f) * current.phC;

            // TODO: currently we set this to zero to prevent unnecessarily
            // heating the brake resistor while the motor(s) are disabled.
            // However this is not technically correct. E.g. if the motor
            // happens to turn sufficiently fast while disabled, current will
            // still flow into the DC bus and it may be desirable to dump this
            // into the brake resistor.
            i_bus = 0.0f; 
            (void) i_bus; 
        }
